...
async function fetchContributors(): Promise<Contributor[]> {
  ...
  try {
    ...
    // If GitHub is still generating stats (202), fall back to contributors endpoint
    ...
    // If stats endpoint is successful
    if (statsRes.ok) {
    ...
    // If stats endpoint fails, try fallback
...
async function fetchContributorsFallback(
  owner: string, 
  repo: string, 
  headers: HeadersInit
): Promise<Contributor[]> {
...
const ContributorCard: React.FC<ContributorCardProps> = ({ 
  ...
  return (
    <div
      className={`${getCardGradient(ratio)} rounded-xl p-6 flex flex-col items-center text-center shadow-sm border transition-all duration-300 hover:transform hover:scale-105`}
    >
