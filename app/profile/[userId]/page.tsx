        ...
        <div className="backdrop-blur-md bg-white/5 rounded-2xl p-6 border border-white/20">
          ...
          {badges.length > 0 ? (
            <div className="space-y-4">
              {badges.map((badgeData, idx) => {
                const badgeName = typeof badgeData === 'string' ? badgeData : badgeData.name || badgeData.badge;
                ...
                return (
                  <div key={idx} className="group hover:scale-[1.02] transition-all duration-300 cursor-pointer">
                    <div className="backdrop-blur-md bg-white/10 rounded-2xl p-4 border border-white/20 hover:border-white/40 hover:bg-white/20 transition-all duration-300 relative overflow-hidden">
                      ...
                      <div className={`absolute inset-0 bg-gradient-to-br ${config.color} opacity-20 group-hover:opacity-30 transition-opacity duration-300`}></div>
                      
                      {/* Content - Row layout for desktop, column for mobile */}
                      <div className="relative z-10 flex flex-col sm:flex-row items-center gap-4">
                        {/* Badge Icon */}
                        <div className="flex-shrink-0">
                          <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${config.color} flex items-center justify-center text-xl shadow-lg group-hover:shadow-xl transition-shadow duration-300`}>
                            {config.icon}
                          </div>
                        </div>
                        ...
                        <div className="flex-shrink-0 text-center sm:text-right">
                          <div className="flex items-center justify-center sm:justify-end gap-2 text-green-400 text-sm font-semibold mb-1">
